...
function buildSchedule(months, total, firstDate){
  ...
  // Build series: start + remaining after each payment
  var next = [];
  var running = total;
  next.push(running);
...
/* ===============================
   WEBCHAT → PAGE (Cognigy bridge listeners)
   =============================== */

// Sign in
window.addEventListener('webapp.signin', function () {
  console.log("[app] webapp.signin received");
  state.authed = true;
  state.eligibility = 'Unknown';
  renderOverview();
  addTranscript('system', 'Signed in via GOV.UK One Login (mock) [from Cognigy].');
});

// Eligibility
window.addEventListener('webapp.eligibility', function (e) {
  console.log("[app] webapp.eligibility received", e && e.detail);
  if (!state.authed) { addTranscript('bot', 'Please sign in first to check eligibility.'); return; }
  var status = (e && e.detail && e.detail.status) ? e.detail.status : 'Likely';
  state.eligibility = status;
  renderOverview();
  addTranscript('bot', 'Eligibility updated: ' + status + ' [from Cognigy].');
  // Optional: auto-show/hide options based on status
  if (status === 'Likely') {
    var first = new Date(); first.setMonth(first.getMonth()+1); first.setDate(1);
    state.options = [
      { months: 6,  monthly: 216.75, firstPayment: first.toISOString(), total: 1240.5 },
    ...
    state.options = [];
    renderOptions();
    document.getElementById('scheduleCard').hidden = true;
    document.getElementById('chartCard').hidden = true;
  }
});

// Show options
window.addEventListener('webapp.options', function (e) {
  console.log("[app] webapp.options received", e && e.detail);
  if (!state.authed) { addTranscript('bot', 'Please sign in first to see plan options.'); return; }
  var first = new Date(); first.setMonth(first.getMonth()+1); first.setDate(1);
  state.options = (e && e.detail && e.detail.options) ? e.detail.options : [
    { months: 6,  monthly: 216.75, firstPayment: first.toISOString(), total: 1240.5 },
    { months: 9,  monthly: 144.50, firstPayment: first.toISOString(), total: 1240.5 },
    { months: 12, monthly: 108.38, firstPayment: first.toISOString(), total: 1240.5 }
  ];
  renderOptions();
  addTranscript('bot', 'Here are some plan options. [from Cognigy]');
});

// *** Select a specific plan (6 / 9 / 12 months) ***
window.addEventListener('webapp.selectPlan', function (e) {
  console.log("[app] webapp.selectPlan received", e && e.detail);
  if (!state.authed) { addTranscript('bot', 'Please sign in first.'); return; }
  var months = (e && e.detail && e.detail.months) ? Number(e.detail.months) : 9;
  var allowed = {6:true,9:true,12:true};
  if (!allowed[months]) { months = 9; }
  var first = new Date(); first.setMonth(first.getMonth()+1); first.setDate(1);
  var total = state.customer.balance || 1240.5;
  buildSchedule(months, total, first.toISOString());
  addTranscript('bot', 'Previewing the ' + months + '-month plan.');
});

/* Optional direct hook the bridge can call */
window.webappSignin = function () {
  console.log("[app] webapp.signin received (direct)");
  state.authed = true;
  state.eligibility = 'Unknown';
  renderOverview();
  addTranscript('system', 'Signed in via GOV.UK One Login (mock) [from Cognigy].');
